...
export function reload_movies(arr, place, genres, bg) {
  place.innerHTML = "";
...
export function reload_movies_all(arr, place, genres, bg) {
  place.innerHTML = "";
  for (let item of arr) {
    let block = document.createElement("div");
    let img = document.createElement("img");
    ...
    hoverBg.onclick = () => {
      let iframe = document.querySelector("#mainFrame");
      let id = item.id;
      getMovieData(`/movie/${id}/videos?language=ru`).then((res) =>
        setTrailers(res.data.results[0], iframe)
      );
    };
    hoverBtn.onclick = () => {
